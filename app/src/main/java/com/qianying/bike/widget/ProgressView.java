...
public class ProgressView extends View {
    ...
    private int strokeSize = 15;//描边宽度
    ...
    protected void onDraw(Canvas canvas) {
        ...
        switch (progressShape) {
            case PROGRESS_ROUND:
                ...
                if (isTextNotEmpty)
                    //绘制文字
                    canvas.drawText(text, centreWidth - textWidth / 2 + strokeSize, centreHeight+smaller/4, mTextPaint);
                break;

            case PROGRESS_RECTANGLE:
                mPath.addRoundRect(rect, smaller / rounded, smaller / rounded, Path.Direction.CW);
                if (isTextNotEmpty)
                    canvas.drawText(text, centreWidth - textWidth / 2 + strokeSize, y, mTextPaint);
                break;
            case PROGRESS_OVAL:
